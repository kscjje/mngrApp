const lectureRefundTemplate = `
<div id="lectureRefundTemplate">
	<div>
...
var popLectureRefund = null;

function createLectureRefundPopup(selector, subPopupSelector, callback) {
	if (popLectureRefund){
		popLectureRefund = null;
		$(selector).dxPopup("dispose");
	}
	
	popLectureRefund = $(selector).dxPopup({
		contentTemplate: $('<div>').append(lectureRefundTemplate),
		visible: true,
		title: '환불정산',
		width:1200,
        height:800,
		...
		dragEnabled: true,
		onShown() {
			createLectureRefundGrid();
			createLectureRefundCondition();
...
function createLectureRefundCondition() {
	$('#lectureRefundTemplate .tab-container').dxTabs({
		dataSource: [{ 
			id: 'tabCard',
			text: '카드/계좌 환불',
		}, { 
		...
		onItemClick(e) {
			...
			if (selectedTab == "tabCard") {
				$("#lectureRefundTemplate .cash-container").hide();
				$("#lectureRefundTemplate .card-container").show();
			}	else {
				$("#lectureRefundTemplate .card-container").hide();
				$("#lectureRefundTemplate .cash-container").show();
			}
		},
		selectedIndex: 0,
	});
	...
	var colCondition2 = [];	
	colCondition2.push({dataField: 'TOTAL_DAYS_CNT', label: {text: '총이용일수',}, editorType:"dxNumberBox",
		editorOptions: {
	...
	colCondition2.push({dataField: 'USE_DAYS_CNT', label: {text: '사용일수',}, editorType:"dxNumberBox",
		editorOptions: {
		    showSpinButtons: true,
		    format: '#,##0',
		    readOnly: true,
	...
	$('#lectureRefundTemplate .refund-info').dxForm({
	...
	$('#lectureRefundTemplate .use-price').dxNumberBox({
	    value: 1000,
	    showSpinButtons: true,
	    format: '#,##0 원',
	    width: 150,
	    onValueChanged() {
	    	var sum_use_price = $('#lectureRefundTemplate .use-price').dxNumberBox("instance").option("value") + $('#lectureRefundTemplate .panalty-price').dxNumberBox("instance").option("value");
	    	
	    	if (sum_use_price > 0) {
	    		sum_use_price = -sum_use_price;
	    	}
	    	
	    	$('#lectureRefundTemplate .sum-use-price').text(sum_use_price);
	    },
	    readOnly: true,
	});
	
	$('#lectureRefundTemplate .panalty-price').dxNumberBox({
	    value: 0,
	    showSpinButtons: true,
	    format: '#,##0 원',
	    width: 150,
...
function createCardCashCondition(selector) {
	$('#lectureRefundTemplate .card-container > div').eq(1).dxButton({
	...
	colCondition.push({dataField: 'USER_TYPE', label: {visible:false}, cssClass:'grp_low_height', editorType:"dxRadioGroup",
		editorOptions:{
			...
			onValueChanged(data) {
				if (data.value == 'CASH') {
					$('#lectureRefundTemplate .cash-container > div').eq(1).hide();
				} else {
					$('#lectureRefundTemplate .cash-container > div').eq(1).show();
					$('#lectureRefundTemplate .cash-container > div').eq(1).dxForm("instance").repaint();
				}
			},    		
		}, 
	});	
	
	$('#lectureRefundTemplate .cash-container > div').eq(0).dxForm({
	    showColonAfterLabel: false,
		colCount: 1,
	    items: colCondition,    	
	});	
	...
	$('#lectureRefundTemplate .cash-container > div').eq(1).dxForm({
	    showColonAfterLabel: false,
		colCount: 2,
	    items: colCondition,    	
	});	
...
function createLectureRefundGrid() {
	...
	$('#lectureRefundTemplate .refund-grid').dxDataGrid({
		...
		keyExpr: 'USER_NO',
		columns: getLectureRefundColumnList(),
		scrolling: {
...
function getLectureRefundColumnList() {
	var resultColumn = {};
	...
	resultColumn = [{
        ...
        buttons: [{
            ...
            onClick: function (e) {
                ...
                if (selectedElement.find(".dx-icon-edit").length > 0) {
                	...
                	$('#lectureRefundTemplate .refund-info').dxForm("instance").getEditor("TOTAL_DAYS_CNT").option("readOnly", false);
                	$('#lectureRefundTemplate .refund-info').dxForm("instance").getEditor("USE_DAYS_CNT").option("readOnly", false);
                	$('#lectureRefundTemplate .use-price').dxNumberBox("instance").option("readOnly", false);
                	$('#lectureRefundTemplate .panalty-price').dxNumberBox("instance").option("readOnly", false);
                } else {
                	selectedElement.find(".dx-icon-save").removeClass("dx-icon-save").addClass("dx-icon-edit");
                	
                	$('#lectureRefundTemplate .refund-info').dxForm("instance").getEditor("TOTAL_DAYS_CNT").option("readOnly", true);
                	$('#lectureRefundTemplate .refund-info').dxForm("instance").getEditor("USE_DAYS_CNT").option("readOnly", true);
                	$('#lectureRefundTemplate .use-price').dxNumberBox("instance").option("readOnly", true);
                	$('#lectureRefundTemplate .panalty-price').dxNumberBox("instance").option("readOnly", true);
                }
            }
        }],		
	}, {		
        ...
        calculateCellValue: function(data) {  
        	if (data.USER_PRIVACY_YN && data.USER_PRIVACY_YN === "Y") {
        		return true;
        	} else {
        		return false;
        	}
        ...
        dataType: "boolean",  
        calculateCellValue: function(data) {  
        	if (data.USER_SEND_YN && data.USER_SEND_YN === "Y") {
        		return true;
        	} else {
        		return false;
        	}
